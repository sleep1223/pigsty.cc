...
而通过 Pigsty 自建的 Supabase 则提供了多达 [**507**](https://pigsty.cc/ext/list/) 个开箱即用的 PG 扩展。
...
| 扩展                                                      | 说明                                                     |
|---------------------------------------------------------|--------------------------------------------------------|
| [`pg_graphql`](https://pigsty.cc/ext/e/pg_graphql/)       | 提供 PG 内的 GraphQL 支持 (RUST)，Rust 扩展，由 PIGSTY 提供                |
| [`pg_jsonschema`](https://pigsty.cc/ext/e/pg_jsonschema/) | 提供 JSON Schema 校验能力，Rust 扩展，由 PIGSTY 提供                     |
| [`wrappers`](https://pigsty.cc/ext/e/wrappers/)           | Supabase 提供的外部数据源包装器捆绑包，Rust 扩展，由 PIGSTY 提供               |
| [`index_advisor`](https://pigsty.cc/ext/e/index_advisor/) | 查询索引建议器，SQL 扩展，由 PIGSTY 提供                                |
| [`pg_net`](https://pigsty.cc/ext/e/pg_net/)               | 用 SQL 进行异步非阻塞 HTTP/HTTPS 请求的扩展 (supabase)，C 扩展，由 PIGSTY 提供 |
| [`vault`](https://pigsty.cc/ext/e/supabase_vault/)        | 在 Vault 中存储加密凭证的扩展 (supabase)，C 扩展，由 PIGSTY 提供            |
| [`pgjwt`](https://pigsty.cc/ext/e/pgjwt/)                 | JSON Web Token API 的 PG 实现 (supabase)，SQL 扩展，由 PIGSTY 提供    |
| [`pgsodium`](https://pigsty.cc/ext/e/pgsodium/)           | 表数据加密存储 TDE，扩展，由 PIGSTY 提供                               |
| [`supautils`](https://pigsty.cc/ext/e/supautils/)         | 用于在云环境中确保数据库集群的安全，C 扩展，由 PIGSTY 提供                        |
| [`pg_plan_filter`](https://pigsty.cc/ext/e/plan_filter/)  | 使用执行计划代价过滤阻止特定查询语句，C 扩展，由 PIGSTY 提供                       |
{.full-width}
...
在这一自建部署架构中，您获得了使用不同内核的自由（PG 15-18，OrioleDB），加装 [**507**](https://pigsty.cc/ext/list/) 个扩展的自由，扩容与伸缩 Supabase / Postgres / MinIO 的自由，
