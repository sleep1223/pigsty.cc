...
1. [<span v-pre>`{{ pg_dbsu }}`</span>](/docs/pgsql/param#pg_dbsu)，默认为 `postgres`
2. [<span v-pre>`{{ pg_admin_username }}`</span>](/docs/pgsql/param#pg_admin_username)，默认为 `dbuser_dba`
...
- 如果 `owner` 参数存在，它作为数据库属主，替代默认的 [<span v-pre>`{{ pg_dbsu }}`</span>](/docs/pgsql/param#pg_dbsu)（通常也就是`postgres`）
- 如果 `revokeconn` 为 `false`，所有用户都有数据库的 `CONNECT` 权限，这是默认的行为。
- 如果显式设置了 `revokeconn` 为 `true`：
  - 数据库的 `CONNECT` 权限将从 `PUBLIC` 中撤销：普通用户无法连接上此数据库
  - `CONNECT` 权限将被显式授予 <span v-pre>`{{ pg_replication_username }}`</span>、<span v-pre>`{{ pg_monitor_username }}`</span> 和 <span v-pre>`{{ pg_admin_username }}`</span>
